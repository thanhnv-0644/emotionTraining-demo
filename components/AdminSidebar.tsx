...
export default function AdminSidebar() {
  ...
  const navItems = [
    ...
    { name: 'Enrollments', icon: 'how_to_reg', href: '/admin/enrollments' },
      ...
      <nav className="flex-1 px-4 py-4 flex flex-col gap-1 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`material-symbols-outlined text-[22px] ${isActive ? 'fill-current' : 'group-hover:text-primary transition-colors'}`}>{item.icon}</span>
        ...
        <Link href="/admin/settings" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors group ${
          pathname === '/admin/settings' ? 'bg-primary/10 text-primary font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'
        }`}>
