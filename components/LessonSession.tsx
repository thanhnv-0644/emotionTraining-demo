...
const EMOTIONS = [
  { id: 'happiness', label: 'Happiness', icon: 'sentiment_very_satisfied', color: 'yellow' },
  { id: 'sadness', label: 'Sadness', icon: 'sentiment_dissatisfied', color: 'blue' },
  ...
  { id: 'neutral', label: 'Neutral', icon: 'sentiment_neutral', color: 'slate' },
];
      ...
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-3 lg:px-10">
        ...
        <div className="flex flex-1 justify-end gap-4 items-center">
          <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full text-primary text-sm font-medium">
            ...
            <span>Response Timer: {String(Math.floor(responseTimer / 60)).padStart(2, '0')}:{String(responseTimer % 60).padStart(2, '0')}</span>
          </div>
          ...
          <div className="max-w-4xl mx-auto w-full flex flex-col gap-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Subject: {currentClip.subject}, Scenario: {currentClip.scenario}
                  </p>
                </div>
                <div className="flex items-center gap-4 text-sm font-medium">
                  <span className="flex items-center gap-1 text-slate-500">
                    <span className="material-symbols-outlined text-base">schedule</span>
                    Duration: <span className="text-primary tabular-nums">{currentClip.duration}</span>
                  </span>
                </div>
              </div>
              <div className="w-full bg-slate-100 dark:bg-slate-800 h-3 rounded-full overflow-hidden">
                <div className="bg-primary h-full rounded-full transition-all duration-500" style={{ width: `${(playbackTime / 105) * 100}%` }}></div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col items-center gap-6">
              ...
              <div className="flex items-center gap-6">
                ...
                >
                  <span className="material-symbols-outlined text-5xl">{isPlaying ? 'pause_circle' : 'play_circle'}</span>
                </button>
                <button className="size-12 rounded-full flex items-center justify-center text-slate-400 hover:text-primary transition-colors">
