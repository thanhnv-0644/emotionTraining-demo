...
export default function CourseLessonsPage() {
  ...
  const lessons = course.lessons.map((lesson, index) => {
    ...
    const lessonProgresses = progresses.filter(p => p.lessonId === lesson.id && p.score !== null);
    const bestScore = lessonProgresses.length > 0
      ? Math.max(...lessonProgresses.map(p => p.score!))
      : undefined;
    return {
      id: lesson.id,
      number: lesson.order,
